...
function getStatusBadge(status?: string) {
  switch (status) {
    case "em_tratamento":
      return <Badge className="rounded-full bg-blue-100 text-blue-800">Em tratamento</Badge>
    case "concluido":
      return <Badge className="rounded-full bg-green-100 text-green-800">Concluído</Badge>
    case "inativo":
      return <Badge className="rounded-full bg-gray-100 text-gray-800">Inativo</Badge>
    default:
      return <Badge className="rounded-full bg-yellow-100 text-yellow-800">Aguardando</Badge>
  }
}

function MetricCard({ label, value, icon, tone = "primary" }: { label: string; value: number; icon: React.ReactNode; tone?: "primary" | "blue" | "green" }) {
  const toneClass = tone === "blue" ? "bg-blue-500/10 text-blue-600" : tone === "green" ? "bg-green-500/10 text-green-600" : "bg-primary/10 text-primary"
      ...
      <div className="min-w-0 flex-1">
        ...
        <main className="overflow-x-hidden p-4 sm:p-6">
          ...
          <div className="mb-6 flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <h1 className="text-2xl font-bold text-foreground sm:text-3xl">Beneficiários</h1>
              <p className="text-muted-foreground">Gerencie beneficiários, acompanhe etapas do atendimento e próximas consultas.</p>
            </div>
          </div>

          <div className="mb-6 grid gap-4 sm:grid-cols-3">
            <MetricCard label="Total" value={stats.total} icon={<Users className="h-6 w-6" />} />
            <MetricCard label="Em tratamento" value={stats.emTratamento} icon={<UserCheck className="h-6 w-6" />} tone="blue" />
            <MetricCard label="Concluídos" value={stats.concluidos} icon={<Clock3 className="h-6 w-6" />} tone="green" />
          </div>
            ...
            <CardHeader>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  ...
                  <CardDescription>{filteredBeneficiaries.length} beneficiário(s) encontrado(s)</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              ...
              {filteredBeneficiaries.length > 0 ? (
                <div className="space-y-4">
                  {filteredBeneficiaries.map((beneficiary) => (
                    <div key={beneficiary.id} className="rounded-2xl border border-border bg-card p-4 transition-all hover:border-primary/30 hover:shadow-md">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex min-w-0 items-start gap-4">
                          ...
                          <div className="min-w-0">
                            <p className="font-black text-foreground">{beneficiary.nome || "Beneficiário"}</p>
                            <p className="text-sm text-muted-foreground">{beneficiary.idade ? `${beneficiary.idade} anos` : "Idade não informada"}</p>
                            ...
                            {beneficiary.voluntario && beneficiary.voluntario !== "-" ? <p className="mt-1 text-sm text-muted-foreground">Voluntário: {beneficiary.voluntario}</p> : null}
                          </div>
                        </div>
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                          {getStatusBadge(beneficiary.status)}
                          <Button size="sm" variant="outline" asChild className="h-10 rounded-full font-bold">
                ...
                <Empty variant="subtle" className="py-12">
                  ...
                  <EmptyDescription>{searchQuery || statusFilter !== "all" ? "Tente ajustar os filtros de busca" : "Nenhum beneficiário cadastrado ainda"}</EmptyDescription>
                </Empty>
              )}
            </CardContent>
          </Card>
